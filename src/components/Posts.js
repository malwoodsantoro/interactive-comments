            ...
            <div className="posts">
              ...
              <StyledReply>
                {replies.length > 0 && replies.map(({ index: replyIndex, user, content, createdAt, id: replyId, score, replyingTo }) => {
                  return (
                    <Post key={replyIndex} username={user.username} score={score} content={content} createdAt={createdAt} currentUser={(currentUser == user.username) ? true : false} current={currentUser} comment={false} replyTo={replyingTo} addReply={addReply} commentId={commentId} replyId={replyId} deletePost={deletePost} editComment={editComment} />
                  )
                }
                )}
              </StyledReply>
